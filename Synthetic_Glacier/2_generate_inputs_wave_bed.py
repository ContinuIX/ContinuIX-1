...
print(f"Bed range (wave): {np.nanmin(bed):.1f} – {np.nanmax(bed):.1f} m")

# === Plot wave bed configuration ===
os.makedirs("./figures", exist_ok=True)

_glacier_mask_plot = ~np.isnan(bed)
_mid_col = cols // 2
_mid_row = rows // 2
_glacier_rows = np.where(_glacier_mask_plot[:, _mid_col])[0]
_glacier_cols = np.where(_glacier_mask_plot[_mid_row, :])[0]
...
im = axes[0].imshow(bed, cmap="terrain", aspect="auto",
                    vmin=np.nanmin(bed), vmax=np.nanmax(bed))
axes[0].set_title("Bed elevation (2D)")
axes[0].set_xlabel("Along-flow (col)")
axes[0].set_ylabel("Cross-flow (row)")
plt.colorbar(im, ax=axes[0], label="Elevation (m)", shrink=0.8)

# ── Cross-flow profile at mid-glacier (x = ~2500 m) ──
axes[1].plot(y_coords[_glacier_rows], bed[_glacier_rows, _mid_col], color="steelblue")
axes[1].set_xlabel("Cross-flow y (m)")
axes[1].set_ylabel("Bed elevation (m)")
axes[1].set_title(f"Cross-flow profile at x ≈ {x_coords[_mid_col]:.0f} m")
axes[1].grid(True, alpha=0.3)

# ── Along-flow profile at glacier centreline (row = mid_row) ──
axes[2].plot(x_coords[_glacier_cols], bed[_mid_row, _glacier_cols], color="darkorange")
axes[2].set_xlabel("Along-flow x (m)")
axes[2].set_ylabel("Bed elevation (m)")
axes[2].set_title("Along-flow profile at centreline")
...
last_h_path  = last_vtu("./output_wave/h/", "h")
last_u_path  = last_vtu("./output_wave/u/", "u")
last_fd_path = last_vtu("./output_wave/flux_div/", "flux_div")
last_dh_path = last_vtu("./output_wave/dhdt/", "dhdt")
...
plt.tight_layout()
os.makedirs("./figures", exist_ok=True)
plt.savefig("./figures/quick_look_wave.png", dpi=150, bbox_inches="tight")
plt.close()
...
fig.suptitle("Perturbation fields — wave bed (seed=42)", fontsize=13)
...
plt.tight_layout()
plt.savefig("./figures/perturbations_wave.png", dpi=150, bbox_inches="tight")
plt.close()
print("Saved → ./figures/perturbations_wave.png")
...
_attrs = {
    "title": "Synthetic half-pipe glacier (wave bed) — steady-state inputs from icepack",
    ...
    "wave_amplitude_m": _wave_amplitude, "wave_sigma_m": _wave_sigma,
    "wave_bump_centers_m": str(_bump_centers),
}
...
def _make_base_ds(extra_attrs=None):
    ...
    return xr.Dataset(
        ...
        data_vars={
            "BED":           _v(bed,          "m",    "Bed elevation (wave)"),
...
_make_base_ds().to_netcdf("./netcdfs/unperturbed_wave.nc")
print(f"unperturbed.nc    →  {os.path.getsize('./netcdfs/unperturbed_wave.nc')/1e6:.0f} MB  "
      f"({len(xr.open_dataset('./netcdfs/unperturbed_wave.nc').data_vars)} vars)")
...
xr.merge([_make_base_ds(extra_attrs={"description": "base fields + all perturbation variants"}),
          ds_pert_extra]).to_netcdf("./netcdfs/perturbed_wave.nc")
print(f"perturbed.nc      →  {os.path.getsize('./netcdfs/perturbed_wave.nc')/1e6:.0f} MB  "
      f"({len(xr.open_dataset('./netcdfs/perturbed_wave.nc').data_vars)} vars)")
...
).to_netcdf("./netcdfs/perturbations_wave.nc")
print(f"perturbations.nc  →  {os.path.getsize('./netcdfs/perturbations_wave.nc')/1e6:.0f} MB  "
      f"({len(xr.open_dataset('./netcdfs/perturbations_wave.nc').data_vars)} vars)")
...
).to_netcdf("./netcdfs/coarsened_wave.nc")
print(f"coarsened.nc      →  {os.path.getsize('./netcdfs/coarsened_wave.nc')/1e6:.0f} MB  "
      f"({len(xr.open_dataset('./netcdfs/coarsened_wave.nc').data_vars)} vars)")
